function compute_motion_trace_crosscorr_and_split_crosscorr()
% Compare MoCo-upper and MoCo-lower motion traces to identify
% lower-jaw-involved motion using cross-correlation.
%
% Logic:
...
% 5) compute normalized cross-correlation only between MoCo-upper and MoCo-lower
% 6) average the 6 UL cross-correlation values
% 7) classify using the averaged UL cross-correlation directly
%
...
% Threshold for the mean UL cross-correlation
% Lower value = more different upper/lower motion
thr_mean_xcorr_ul = 0.6098;

% Maximum lag allowed in cross-correlation (in shots)
maxLag = 3;
...
% Output files
outMat   = fullfile(rootDir, 'motion_trace_ULsplit_xcorr_mean_results.mat');
outXlsx  = fullfile(rootDir, 'motion_trace_ULsplit_xcorr_mean_results.xlsx');
...
XCorr_UL_Tra1 = nan(nCases,1);
XCorr_UL_Tra2 = nan(nCases,1);
XCorr_UL_Tra3 = nan(nCases,1);
XCorr_UL_Rot1 = nan(nCases,1);
XCorr_UL_Rot2 = nan(nCases,1);
XCorr_UL_Rot3 = nan(nCases,1);

Mean_xcorrUL = nan(nCases,1);

WorstXCorrUL = nan(nCases,1);
SecondWorstXCorrUL = nan(nCases,1);
BestXCorrUL = nan(nCases,1);
Worst_Dim_Name = strings(nCases,1);
SecondWorst_Dim_Name = strings(nCases,1);
Best_Dim_Name = strings(nCases,1);

BestLag_UL_Tra1 = nan(nCases,1);
BestLag_UL_Tra2 = nan(nCases,1);
BestLag_UL_Tra3 = nan(nCases,1);
BestLag_UL_Rot1 = nan(nCases,1);
BestLag_UL_Rot2 = nan(nCases,1);
BestLag_UL_Rot3 = nan(nCases,1);
...
for icase = 1:nCases
    ...
    try
        ...
        % ---- Compute UL cross-correlations only ----
        xcorrUL = nan(1,6);
        bestLagUL = nan(1,6);
        ...
        for idim = 1:6
            ...
            [xcorrUL(idim), bestLagUL(idim)] = safe_xcorr_max(xU, xL, maxLag);
        end

        mean_xcorrUL = mean(xcorrUL, 'omitnan');

        % ---- Dimension ranking ----
        % Lower xcorr means more different, so ascending sort
        [sortedXCorrUL, sortIdx] = sort(xcorrUL, 'ascend', 'MissingPlacement', 'last');

        worstXCorrUL = sortedXCorrUL(1);
        worst_dim_name = string(dimNames{sortIdx(1)});

        if numel(sortedXCorrUL) >= 2
            secondWorstXCorrUL = sortedXCorrUL(2);
            secondWorst_dim_name = string(dimNames{sortIdx(2)});
        else
            secondWorstXCorrUL = NaN;
            secondWorst_dim_name = "";
        end

        [bestXCorrUL, bestIdx] = max(xcorrUL);
        best_dim_name = string(dimNames{bestIdx});
        ...
        % ---- Classification: use mean cross-correlation ----
        if isfinite(mean_xcorrUL) && (mean_xcorrUL <= thr_mean_xcorr_ul)
        ...
        XCorr_UL_Tra1(icase) = xcorrUL(1);
        XCorr_UL_Tra2(icase) = xcorrUL(2);
        XCorr_UL_Tra3(icase) = xcorrUL(3);
        XCorr_UL_Rot1(icase) = xcorrUL(4);
        XCorr_UL_Rot2(icase) = xcorrUL(5);
        XCorr_UL_Rot3(icase) = xcorrUL(6);

        BestLag_UL_Tra1(icase) = bestLagUL(1);
        BestLag_UL_Tra2(icase) = bestLagUL(2);
        BestLag_UL_Tra3(icase) = bestLagUL(3);
        BestLag_UL_Rot1(icase) = bestLagUL(4);
        BestLag_UL_Rot2(icase) = bestLagUL(5);
        BestLag_UL_Rot3(icase) = bestLagUL(6);

        Mean_xcorrUL(icase) = mean_xcorrUL;
        WorstXCorrUL(icase) = worstXCorrUL;
        SecondWorstXCorrUL(icase) = secondWorstXCorrUL;
        BestXCorrUL(icase) = bestXCorrUL;

        Worst_Dim_Name(icase) = worst_dim_name;
        SecondWorst_Dim_Name(icase) = secondWorst_dim_name;
        Best_Dim_Name(icase) = best_dim_name;
        ...
        results(icase).xcorrUL = xcorrUL;
        results(icase).bestLagUL = bestLagUL;
        results(icase).mean_xcorrUL = mean_xcorrUL;
        results(icase).worstXCorrUL = worstXCorrUL;
        results(icase).secondWorstXCorrUL = secondWorstXCorrUL;
        results(icase).bestXCorrUL = bestXCorrUL;
        results(icase).worst_dim_name = worst_dim_name;
        results(icase).secondWorst_dim_name = secondWorst_dim_name;
        results(icase).best_dim_name = best_dim_name;
        results(icase).motionPattern = patternLabel;

        fprintf('Case %s done.\n', caseID);
        fprintf('  mean xcorr(U,L)       = %.4f\n', mean_xcorrUL);
        fprintf('  worst xcorr(U,L)      = %.4f (%s)\n', worstXCorrUL, worst_dim_name);
        fprintf('  2nd worst xcorr(U,L)  = %.4f (%s)\n', secondWorstXCorrUL, secondWorst_dim_name);
        fprintf('  best xcorr(U,L)       = %.4f (%s)\n', bestXCorrUL, best_dim_name);
...
T_summary = table( ...
    CaseID, ...
    XCorr_UL_Tra1, XCorr_UL_Tra2, XCorr_UL_Tra3, ...
    XCorr_UL_Rot1, XCorr_UL_Rot2, XCorr_UL_Rot3, ...
    BestLag_UL_Tra1, BestLag_UL_Tra2, BestLag_UL_Tra3, ...
    BestLag_UL_Rot1, BestLag_UL_Rot2, BestLag_UL_Rot3, ...
    Mean_xcorrUL, ...
    WorstXCorrUL, SecondWorstXCorrUL, BestXCorrUL, ...
    Worst_Dim_Name, SecondWorst_Dim_Name, Best_Dim_Name, ...
...
save(outMat, 'results', 'T_summary', ...
    'thr_mean_xcorr_ul', 'maxLag', 'outlier_z_thr', 'dimNames');

writetable(T_summary, outXlsx, 'Sheet', 'Summary');

% Detailed sheet
detailedCell = cell(nCases, 13);
header = [{'CaseID'}, ...
          strcat('xcorrUL_', dimNames), ...
          strcat('bestLagUL_', dimNames)];

for i = 1:nCases
    detailedCell{i,1} = char(CaseID(i));
    if isfield(results(i), 'xcorrUL')
        detailedCell(i, 2:7)   = num2cell(results(i).xcorrUL);
        detailedCell(i, 8:13)  = num2cell(results(i).bestLagUL);
    else
...
writetable(T_detail, outXlsx, 'Sheet', 'UL_XCorr');
...
%% =========================================================
function [rmax, bestLag] = safe_xcorr_max(x, y, maxLag)
% Return the maximum normalized cross-correlation coefficient
% within +/- maxLag, and the lag where it occurs.
...
% remove mean to focus on shape similarity
x = x - mean(x);
y = y - mean(y);
...
rmax = max(min(rmax, 0.999999), -0.999999);
end
