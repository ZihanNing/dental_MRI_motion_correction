...
caseFoldersAll = d(isCase);

if isempty(caseFoldersAll)
    error('No case folders found under: %s', rootDir);
end

% Keep only folders with at least one digit in the name
hasDigit = cellfun(@(s) ~isempty(regexp(s, '\d', 'once')), {caseFoldersAll.name});
caseFoldersAll = caseFoldersAll(hasDigit);

if isempty(caseFoldersAll)
    error('No numeric case folders found under: %s', rootDir);
end

% Sort by numeric part if possible
caseNamesAll = {caseFoldersAll.name};
...
[~, idxSort] = sortrows([isnan(caseNumsAll(:)), caseNumsAll(:)], [1 2]);
caseFoldersAll = caseFoldersAll(idxSort);
caseNumsAll    = caseNumsAll(idxSort);

% Optional subset selection
if isempty(selectedCases)
    caseFolders = caseFoldersAll;
else
    keep = ismember(caseNumsAll, selectedCases);
    caseFolders = caseFoldersAll(keep);
