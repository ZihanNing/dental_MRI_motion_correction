...
modality=[2 10];%Modify to specify a particular modality (see list in Control/modalList.m)
...
for l=n0:nd:ne
    ...
    pathOu=fullfile(outRootPath,path{n},'Re-Se');
    if ~exist(pathOu,'dir')
        fprintf('\nDWI %d of %d (id %d)\n',l,length(path),n);
        protoPipeline(path{n},modality,outRootPath,inpRootPath,series);%RECONSTRUCTING
        procePipeline(path{n},modality,outRootPath,series);%PREPROCESSING
