...
assert(all(N(1:ND)>=w),'hankel:: Dimensions of data%s smaller than dimensions for calibration%s',sprintf(' %d',N(1:ND)),sprintf(' %d',w));

y=zeros([prod(N(1:ND)-w+1) prod(w) N(ND+1:end)],'like',x);
NY=size(y);NY(end+1:3)=1;

%%% CREATE HANKEL INDICES
iw=1:prod(w);
sw=ind2subV(w,iw);
iX=cell(1,ND);
for s=1:ND
...
iXi=cell(1,ND);
for n=1:prod(w)%YB: similar as in Miki Lustig's code, shifting of the calibration matrix instead of sliding window over it
