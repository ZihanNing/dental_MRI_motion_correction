...
for n=1:max(nIt)  
    ...
    if any(isnan(xup(:)));fprintf('Break out of CGsolver due to NaN detection.\n');break;end
    x=x+xup;
    ...
    if err<tol ;fprintf('Break out of CGsolver due to error tolerance reached.\n');break;end 
    if all(abs(rs(:))<1e-6);fprintf('Break out of CGsolver due to rs<1e-6.\n');break;end 
end
nn=n;
