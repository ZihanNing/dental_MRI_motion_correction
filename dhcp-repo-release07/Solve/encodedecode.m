    ...
    if isfield(E,'Tr') && ~isempty(E.Tr)
        if any(vA<=E.NMs)
            ...
            if any(E.Tr(:)~=0)
                ...
                if isfield(E,'Dc');xT=bsxfun(@times,xT,dephaseRotation(dynInd(Tr,E.Dc.d,6),E.Dc.D));end   % Linear model based on DCT atoms - Lucilio
                ...
                %if isfield(E,'Ds');xT=bsxfun(@times,xT,dephaseRotationSusc(dynInd(Tr,E.Ds.d,6),E.Ds.chi,E.Ds.TE));end   % Susceptibility model with voxel basis
                ...
                if isfield(E,'intraVoxDeph') && E.intraVoxDeph && isfield(E,'Dl')&&isfield(E,'Dl');xT=bsxfun(@times,xT,intravoxDephasing(Tr,dynInd(E.Db.cr,vA(vA<=E.NMs),5),size(xT), E.Db,E.Dl));end
                %TRANSFORM
    ...
    if isfield(E,'Tr') && ~isempty(E.Tr)    
        ...
        if any(vA<=E.NMs)    
            ...
            if any(E.Tr(:)~=0)     
                ...
                %DEPHASING - motion dependent
                if isfield(E,'intraVoxDeph') && E.intraVoxDeph && isfield(E,'Dl')&&isfield(E,'Dl');xinT=bsxfun(@times,xinT,intravoxDephasing(Tr,dynInd(E.Db.cr,vA(vA<=E.NMs),5),size(xT), E.Db,E.Dl));end
