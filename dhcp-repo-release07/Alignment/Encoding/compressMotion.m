...
if NT(5)>1
    ...
    while n<=NT(5)
        ...
        while m+n<=NT(5)
            ...
            if any(abs(Tnex(:)-Tcur(:))>(-1)^disableGrouping * 1e-6);break;end %YB: when setting this negative, you will not group anything
            m=m+1;
        end
        indGroup(n:n+m-1)=m;
        n=m+n;
    ...
    %------YB: dealing with empty shots for motion as well ----STOP
    for f=1:2
