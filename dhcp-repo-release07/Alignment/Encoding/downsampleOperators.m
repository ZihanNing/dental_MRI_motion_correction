function [xRes,SRes,yRes,ARes,MRes,rGridRes,kGridRes,rkGridRes,F,FH,fail]=downsampleOperators(l,x,S,y,A,M,gibbsRing,fo,gpuIn,useFilter, MS)
...
if nargin<11 || isempty(MS);MS=[];end%YB
...
if useFilter==1 && gibbsRing~=0 && gibbsRing<=1;HY=buildFilter(NYRes(1:3),'tukeyIso',[],gpuIn,gibbsRing);elseif useFilter==1 && all(l~=0);HY=buildFilter(NYRes,'CubicBSpline',[],gpuIn);else; HY=[];end
if ~isempty(HY)
    if ~fo;yRes=filtering(yRes,HY);else; yRes=bsxfun(@times,yRes,HY);end
end

if any(size(M)==NXRes);M = single(M);end
...
if ~isempty(MS)
    fac = MS/min(MS(:))
    NX = NX.*fac
    fprintf('downsampleOperators:: Correcting for the anisotropy.');
end
