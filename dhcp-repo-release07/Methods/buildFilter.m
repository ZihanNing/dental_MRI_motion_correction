...
if nargin<5 || isempty(gibbsRing);gibbsRing=0;end%YB: so no transition band and hard filtering
...
%%RGRID...
kGrid=generateGrid(N,gpu,2*1i*pi./sp,ceil((N+1)/2));%YB: Why deviding by sp?
...
elseif strcmp(typ,'1stFiniteDiscreteForward')%First order forward finite differences
    ...
    for n=1:ND
        if N(n)>1;rGrid{n}(1)=1;rGrid{n}(end)=-1;rGrid{n}=-rGrid{n}/(sp(n));end %YB: specifies kernel for convolution  - will be Fourier transformed at the end
    end
elseif strcmp(typ,'1stFiniteDiscreteBackward')%First order backwards finite differences
    assert(all(~c),'1st Order Finite Discrete Difference filter not defined for DCT\n');
    for n=1:ND
...
x=ifftshift(x); %YB: returned filter is in the ishifted domain (corners)
if any(c)%We assume the filter is symmetric
    ...
    x=dynInd(x,v,1:ND); % YB: For the cosine domain filter, you extract only a fraction of the filter (so filter size is smaller)
end
