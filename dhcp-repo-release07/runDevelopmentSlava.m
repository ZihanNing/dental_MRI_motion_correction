...
modality=[2 9];%6;%5;%[];%5:6;%[];%[];%[2:6];%[2 9];%[];%[];%10;%2:7;%10;%[];%[9 10];%[];%[2 5];%2;%[2 5];%[];%[2 10];%[];%10;%[];%[];%6;%[];%[];%2:5;%[];%2:5;%1:8;%[];%5:7;%1:7;%7;%[];%10;%[];%5;%[];%2:8;%10;%7;%[];%10%[];%[];%2:5;%[];%[9 10];%[9 10];%[2 5];%[2 5];%[2 5];%[];%[];%2:5;%[];%All modalities. Modify to specify a particular modality (see list in Control/modalList.m)
series=[];%15;%[];%[];%[];%[21 25];%[];%[9 8 7];%15:16;%[];%[];%[];%6;%8;%6;%9:100;%[];%[];%16:100;%[];
...
repeated=0;
caseFetal=0;
%shaRootPath='/home/lcg13/Data/pnrawDe/FetalRelease06/PipelineTests2';
...
%end
for n=1:length(path);fPro=procePipelineTransform(path{n},modality,outRootPath,series,'Sh');end
...
%for n=1:length(path);[fPro,~]=procePipelineAssembleSVR(path{n},5:6,outRootPath,series);end
