...
function [E, En] = GDsolverSuscValue(y,E,EH,A,C,R,x,nIt, tol, deb,Mtissue)
...
if isfield(E.Ds,'Optim') && isfield(E.Ds.Optim, 'alphaList');   alphaList=E.Ds.Optim.alphaList;else alphaList=10.^(-1* [-4:0.7:6]); end%line search step size
...
%Initialise the 2 values
E.Ds.suscVal = zeros([2,1],'like',E.Ds.chi);
E.Ds.suscVal(1) = multDimMea( E.Ds.chi(~Mtissue)  );%air 
E.Ds.suscVal(2) = multDimMea( E.Ds.chi(Mtissue)  );%tissue 
E.Ds.chi = E.Ds.suscVal(1)*single(~Mtissue) + E.Ds.suscVal(2)*single(Mtissue);

%%% ITERATE
for n=1:max(nIt)    
    chi = E.Ds.chi; % Set original value aside so no need to duplicate E
    suscVal = E.Ds.suscVal;
...
upSuscVal = zerosL(E.Ds.suscVal);
upSuscVal(1) = multDimSum(update(~Mtissue));
upSuscVal(2) = multDimSum(update(Mtissue));
update = upSuscVal(1)*single(~Mtissue) + upSuscVal(2)*single(Mtissue);
        ...
        E.Ds.suscVal = suscVal + upSuscVal;
