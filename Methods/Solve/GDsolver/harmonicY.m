...
%      P = legendre(n,cos(th),'unnorm'); %YB test
%      a = (2*n+1)*factorial(n-m);
%      b = 4*pi*factorial(n+m);
%      C = sqrt(a/b);
     
    if m ~= 0 %YB: according to cheb: add sqrt(2)
        C = sqrt(2)*C;
    end
else
...
if issolid
    ...
    Csolid = sqrt(2*n+3); %YB: normalisation
    isirregular = 0; %YB: allow the irregular solid harmonics    
    
    if isirregular %irregular soldi harmonic - singular at origin
        idx = r >0;
        Y(idx) = Csolid * Y(idx) ./ (r(idx).^(n+1));
        
    else %regular soldi harmonic - vanishes at origin 
        Y = Csolid .* Y .* r.^n;
    end
