...
function [E, x, En] = GDsolverB1(y,E,EH,A,C,R,x,nIt,tolType,tol, deb)
...
%%% INITIALISE
gpu = isa(x,'gpuArray');
Ncr = size(E.B1m.cr); Ncr(end+1:6) =1;
NX = size(x);
NStates = E.NMs; 
NSegments = E.NSe; %inlcudes the outer segment (shutter)

if isfield(E.B1m,'Optim') && isfield(E.B1m.Optim, 'BlSzCoef');BlSzCoef = E.B1m.Optim.BlSzCoef; else BlSzCoef=5 ;end % Block size for coefficient gradient calculation
if isfield(E.B1m,'Optim') && isfield(E.B1m.Optim, 'BlSzStates');BlSzStates = E.B1m.Optim.BlSzStates; else BlSzStates=E.bS(1) ;end% Block size for coefficient gradient calculation
if isfield(E.B1m,'Optim') && isfield(E.B1m.Optim, 'meanC');meanC = E.B1m.Optim.meanC; else meanC=0 ;end
if isfield(E.B1m,'Optim') && isfield(E.B1m.Optim, 'deTaylor');deTaylor = E.B1m.Optim.deTaylor(1); else deTaylor=2 ;end%Note E.B1m.Optim.deTaylor(2) contains information about outer iterations (handled in solveXTB_ext)
if isfield(E.B1m,'Optim') && isfield(E.B1m.Optim, 'Mbg');Mbg = E.B1m.Optim.Mbg; else Mbg=1:NX(3);end
if isfield(E.B1m,'Optim') && isfield(E.B1m.Optim, 'alphaList');alphaList=E.Dl.Optim.alphaList;else alphaList=10.^(-1* [1:1.3:8]); range = alphaList(1)/alphaList(end); end
...
deTaylor=2; %B1 map not shot dependent!
meanC=0; %No reason to be zero average
alphaList=10.^(-1* [-1:1.3:15]);
...
for n=1:max(nIt)    
    cr = E.B1m.cr; % Set coeffients aside so E does not have to be duplicated
    res = y-encode(x,E);
    
    %%%GRADIENT INITIALISATION
    Ng = Ncr; Ng(5) = NSegments;
    grad =  zeros(Ng, 'like', real(E.B1m.cr)); if gpu; grad = gpuArray(grad);end % Make sure g is not complex or it will raise errors
    ...
    for a=1:BlSzStates:NStates %Chunk states otwerwise is has size [Nx Nstates] and this runs out of memmory depending on NX and NStates
        ...
        for ii = 1:BlSzCoef:Ncr(6) %number of basis coefficients 
            ...
            gTB = bsxfun(@times, gT, reshape(E.B1m.B(:,vI), [NX 1 1 length(vI)]));%B1 modulation not performed in decode 
            gTB = sincRigidTransform(gTB,Tf,0,E.Fof,E.Fob);%inverse transform
            
            %dephasing in scanner reference frame - a;ready done in decode
            if isfield(E,'intraVoxDeph') && E.intraVoxDeph && isfield(E,'Db')&&isfield(E,'Dl');gTB=bsxfun(@times,gTB,intravoxDephasing(Tr,dynInd(E.B1m.cr,vA(vA<=E.NMs),5),size(gTB), E.B1m,E.Dl));end
            ...
            gTB = -1*bsxfun( @times , conj(x), gTB); % -1 since 1i now does not cancel negative sign when going out of conjugate
    ...
    if deTaylor==2;grad = multDimSum(grad,5);end%No motion dependance!!    
    ...
    for alpha = alphaList
        ...
        if deTaylor==2
            ...
            E.B1m.cr =  cr + update;%multDimSum(bsxfun(@times, permute(E.Dl.f(E.Dl.Tr),[1:5 7 6]),update),7);
        else
