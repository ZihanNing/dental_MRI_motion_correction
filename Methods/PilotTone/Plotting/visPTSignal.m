...
if nargin < 8 || isempty(numFig);numFig=[];end
...
%%% PLOT
h=createFig(numFig);
if numFig>0;set(h,'Color',[0 0 0]+invCol,'Position', get(0,'Screensize'));end

if ismember(plotType,[1 3]) %Magnitude and phase
    %Magnitude
    subplot(NPlot(1),NPlot(2),1); plot(time, abs(p),'LineWidth',LineWidth,'LineStyle','-')
    if numFig>0;set(gca,'Color','none','XColor',[1 1 1]*(1-invCol),'YColor',[1 1 1]*(1-invCol),'FontSize',FontSize+ModTick,'TickLabelInterpreter','latex');end
...
if ~isempty(outlWePT) && ~isempty(stateSample)
    subplot(NPlot(1),NPlot(2),1);%Only plot on first one
    stateSample = sortmSt(stateSample);
