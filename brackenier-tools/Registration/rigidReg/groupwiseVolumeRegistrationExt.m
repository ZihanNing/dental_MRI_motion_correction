...
function [x,T,y,xx]=groupwiseVolumeRegistrationExt(x,W,T,meanT,lev,fracOrd,parComp,xx,tolAccel,deb)
...
BlSz=10;
for l=1:length(lev) 
    if deb;fprintf('Resolution level: %d\n',lev(l));end
    NXres=round(NXV/lev(l));
    [~,kGrid,rkGrid]=generateTransformGrids(NXV,gpu,NXres,centerIdx(NXres),1);
    ...
    while fina~=2
        ...
        while fina==0
            ...
            flagw(E<=Eprev)=2;               
            if deb;fprintf('Energy before: %0.6g / Energy after: %0.6g\n',sum(Eprev),sum(E));end
            if any(flagw==1 | flagw==0)  
                ...
                if deb
                    ...
                    fprintf('/ Maximum change in rotation (deg): ');fprintf('%0.3f ',max(rotMax,[],1));
                    fprintf('\n');             
                end
                ...
                if deb;fprintf('Not converged motion states: %d of %d\n',NT(ndT-1)-sum(single(convT)),NT(ndT-1));end
            end
        end
...
[~,kGrid,rkGrid]=generateTransformGrids(NXV,gpu,[],centerIdx(NXV),1);
