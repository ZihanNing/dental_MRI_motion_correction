...
function [xo,T] = alignVolumes_new(xi, res, orient, ref, exclDim, excludeRef, padding, debug)
...
%   * {DEBUG} to show intermediate results and the region to be excluded during registration (defaults to 0). If 1, show area excluded. If 2 show all intermediate results
...
if applyToOther
    ...
    xi = dynInd(xi,1,1); %take out the volumes from x that are used for regisrtation
    ...
    %%%Check if number of arrays to transforms are same for every pose
...
for vol =1:size(xi,2)
    if ~isempty(orient{vol})
        [permMT{vol},flMT{vol}] = MT2perm(orient{vol}); permMT{vol}(end+1:12) = length(permMT{vol})+1:12;
...
dimStore = 5;%Fift dimension so easily applicable to 4D arrays of xToTransform
pyr=[8 4 2 1];
tolAccel=0.01;
fraction_order = 0.5;
w = cat(dimStore,xi{:});
...
if debug>1; plotND([],permute(abs(w),[1:3 5 4]),[],[],0,{[],1},[],[],[],200); sgtitle('alignVol.m:: Before alignment');end
...
if debug>1; plotND([],permute(abs(w),[1:3 5 4]),[],[],0,{[],1},[],[],[],201); sgtitle('alignVol.m:: After alignment');end
...
%%% Permute arrays so that different poses (now registered) are in 4th dimensions and the arrays to be transformed (e.g. fMRI) in the 5th dimension
%xo = permute(xo, [1:3 5 4]);
