        ...
        for k = 1:kMax
            ...
            if blockCtr == blockSz || k == kMax || (isBrokenRead && blockCtr > 1)
                ...
                if this.flagAverageDim(1)
                    block = mean(block, 1);
                %elseif ~isequal(selRange{1},':')%YB: needed to fix strcmp because ASCII code for ':' = 58 so channel 58   was crashing
                ...
                if this.flagAverageDim(2)
                    ...
                    %if ~isequal(selRange{2},':') %YB: needed to fix strcmp because ASCII code for ':' = 58 so channel 58   was crashing
                    if ~strcmp(selRange{2},':')
