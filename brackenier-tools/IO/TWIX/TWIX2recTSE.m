...
function rec = TWIX2recTSE(TW, suppFOV, isPT, shimDir, removeOversampling, noiseFileName, fileName, resRec, useGPU, removeZeroSamples, targetR, idTE, idINV)
...
if isShotBased
    ...
    Lin = dynInd(Lin,idFullInversion,2); Par = dynInd(Par,idFullInversion,2);idFullInversion=[];
...
NImageCols = TW.hdr.Config.BaseResolution%TSE
...
NKSpaceColsGrid = TW.hdr.Config.NColMeas %TSE
% NKSpaceLinsGrid = TW.hdr.Meas.iPEFTLength;
% NKSpaceParsGrid = TW.hdr.Meas.i3DFTLength;
NKSpaceLinsGrid = TW.hdr.Config.NImageLins/2*(1+PEOverSamp(1))*TW.hdr.MeasYaps.sKSpace.dPhaseResolution%TSE
...
hApod = fftshift(buildFilter(size(rec.y,2),'tukeyIso',.5,[],.5)); 
rec.y = rec.y.*hApod(:).';
