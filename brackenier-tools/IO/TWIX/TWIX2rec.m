...
function rec = TWIX2rec(TW, suppFOV, isPT, shimDir, removeOversampling, noiseFileName, fileName, resRec, useGPU, removeZeroSamples, targetR, idTE, idINV)
...
rec=[];
if exist('Date', 'var'); rec.Names.Date=Date; end % ZN: for some version of raw data, this parameter might not exist
...
elseif isMP2RAGE % ZN: enable this for MP2RAGE
    if TW.hdr.MeasYaps.sKSpace.unReordering==1%Linear
        TFE = TW.image.dataSize(4); 
        TFE = findTFE(TW,3);
    else %Linear-Rotated
        TFE = TW.image.dataSize(3);
        TFE = findTFE(TW,2);
    end
...
%     NKSpaceLinsGrid = TW.hdr.Config.NImageLins*(1+PEOverSamp(1))*TW.hdr.MeasYaps.sKSpace.dPhaseResolution;
    NKSpaceLinsGrid = TW.hdr.Meas.iPEFTLength*(1+PEOverSamp(1))*TW.hdr.MeasYaps.sKSpace.dPhaseResolution;
%     NKSpaceParsGrid=TW.hdr.Meas.lImagesPerSlab*(1+PEOverSamp(2))*TW.hdr.MeasYaps.sKSpace.dSliceResolution %SPACE
    NKSpaceParsGrid=TW.hdr.Meas.i3DFTLength*(1+PEOverSamp(2))*TW.hdr.MeasYaps.sKSpace.dSliceResolution; %SPACE
end
...
if (rec.Enc.UnderSampling.Flag || ~isempty(targetR)) && removeZeroSamples
    ...
    %Assign autocalibration lines to rec.ACS % added by ZN
...
else % added by ZN % ZN: for fully-sampled dataset, if you would like to change RTarget as well
    rec.Enc.UnderSampling.R = [1 1]; % ZN: the dataset is fully-sampled
    if ~isempty(targetR);rec.Enc.UnderSampling.RTarget = targetR; end % ZN: still enable RTarget for fully-sampled dataset
end

% disabling this, since it has been added above by ZN
...
%% NOISE READOUT
if existsFileVar(noiseFileName) || existsFileVar(strcat( noiseFileName,'.dat')) %Take noise samples from other data
...
if isPT
    ...
    rec.PT = pilotToneConversion(rec.PT);
    ...
    yProjRO = fillCentPT (rec.PT.yProjRO, 1, 0, rec.PT.extraction.facFOVTh);
    [~,rec.PT.idxRO] = max(yProjRO,[],1);
    rec.PT.idxRO = round(multDimMea(rec.PT.idxRO,5:ND));
    ...
    ParTemp=Par;LinTemp=Lin;
    if rec.Enc.UnderSampling.Flag && removeZeroSamples 
        LinTemp = ceil(LinTemp/rec.Enc.UnderSampling.R(1));
        ParTemp = ceil(ParTemp/rec.Enc.UnderSampling.R(2));
    end
    %Extract
    if rec.PT.extraction.sincInterp.Flag%PT signal is peak of the sinc-interpolated signal
        fprintf('Using sinc interpolation to extract the PT signal.\n');
        dimImageDomain = 1;
        if rec.PT.deModulate.Flag && rec.PT.extraction.sincInterp.useDemodParam
            fprintf('Using sinc interpolation to de-modulate imaging k-space combined with PT extraction.\n');
            [rec.y, rec.PT.pSliceImage] = demodulatePTLeakage(rec.y, 1, rec.PT.extraction.sincInterp.padFac,  rec.PT.extraction.sincInterp.windowCent, rec.PT.extraction.sincInterp.windowWidth, [], rec.PT.extraction.facFOVTh,dimImageDomain,rec.PT.extraction.sincInterp.averageFreqCoils,LinTemp,ParTemp);
        elseif rec.PT.deModulate.Flag
            fprintf('Using sinc interpolation to de-modulate imaging.\n');
            [~, rec.PT.pSliceImage] = demodulatePTLeakage(rec.y, 1, rec.PT.extraction.sincInterp.padFac,  rec.PT.extraction.sincInterp.windowCent, rec.PT.extraction.sincInterp.windowWidth, [], rec.PT.extraction.facFOVTh,dimImageDomain,rec.PT.extraction.sincInterp.averageFreqCoils,LinTemp,ParTemp);
            fprintf('Using sinc interpolation to extract PT signal.\n');
            rec.y = demodulatePTLeakage(rec.y, 1, rec.PT.deModulate.padFac,  rec.PT.deModulate.windowCent, rec.PT.deModulate.windowWidth, [], rec.PT.deModulate.facFOVTh,dimImageDomain,rec.PT.deModulate.averageFreqCoils,LinTemp,ParTemp);
        else
            fprintf('Using sinc interpolation to de-modulate imaging k-space.\n');
            [~, rec.PT.pSliceImage] = demodulatePTLeakage(rec.y, 1, rec.PT.extraction.sincInterp.padFac,  rec.PT.extraction.sincInterp.windowCent, rec.PT.extraction.sincInterp.windowWidth, [], rec.PT.extraction.facFOVTh,dimImageDomain,rec.PT.extraction.sincInterp.averageFreqCoils,LinTemp,ParTemp);
        end
        rec.PT.offsetMBHz=0;
        rec.PT.idxMB=1;
    else%PT signal can be multiband signal
        fprintf('Using biggest element in hybrid domain to extract the PT signal.\n');
        ...
        %Demodulate k-space leakage   
        if rec.PT.deModulate.Flag
            fprintf('Using sinc interpolation to de-modulate imaging k-space.\n');
            dimImageDomain = 1;
            plotND([],dynInd(rec.y,3,4),[0 2e-3]);
            rec.y = demodulatePTLeakage(rec.y, 1, rec.PT.deModulate.padFac,  rec.PT.deModulate.windowCent, rec.PT.deModulate.windowWidth, [], rec.PT.deModulate.facFOVTh,dimImageDomain,rec.PT.deModulate.averageFreqCoils,LinTemp,ParTemp);      
        end
    end

    %Recursive call for de-modulation
    if rec.PT.deModulate.Flag && rec.PT.deModulate.NRep>1
        for ii=1:rec.PT.deModulate.NRep-1
            fprintf('Calling additional de-modulation of the imaging k-space.\n');
            rec.y = demodulatePTLeakage(rec.y, 1, rec.PT.deModulate.padFac,  rec.PT.deModulate.windowCent, rec.PT.deModulate.windowWidth, [], rec.PT.deModulate.facFOVTh,dimImageDomain,rec.PT.deModulate.averageFreqCoils,LinTemp,ParTemp);
            plotND([],dynInd(rec.y,3,4),[0 2e-3]);
        end
    end
    
    if rec.PT.deModulate.applyApod
       fprintf('Applying apodisation in readout direction to suppress the PT signal.\n'); 
       rec.PT.hApod = makePTApod(size(rec.y,1), rec.PT.factorFOV, rec.PT.deModulate.facFOVTh); 
       rec.y = rec.y.*rec.PT.hApod;
    end
    
    %Average Multiband frequencies
    if rec.PT.preProcessing.isAveragedMB && rec.PT.extraction.sincInterp.Flag==0
    ...
    if rec.PT.preProcessing.isRelativePhase%This in hybrid k-space, as it is supposed to be for phase subtraction
        rec.PT.pSliceImage = rec.PT.pSliceImage .* conj( exp(1i*angle(dynInd(rec.PT.pSliceImage,rec.PT.preProcessing.relativePhaseCoilIdx,4)))); 
    end
end
...
%% PLOT PT SIGNAL
if isPT
    line = multDimSum(abs(squeeze(rec.PT.yProjRO)),[2:4]).';
    peakId = zerosL(line); peakId(rec.PT.idxRO)=1;
    visPTSignal (line, [], [],[], 1, peakId, 1:length(line), 199, replace(sprintf('\\textbf{Central line hybrid k-space (mean across coils):}\n %s',acqName),'_',' '), [], [], 'Image readout [\# voxels]',{'y_{hybrid}','a.u.'});
...
if ~removeOversampling; rec.Enc.FOVmm(1) = rec.Enc.FOVmm(1)*2; end % ZN: consider the not removing over-sampling cases
...
if ~removeOversampling; rec.Enc.AcqSize(1) = NY(1); end % ZN: take care of the not over sampling removal case
...
if isShotBased
    ...
    if isMP2RAGE; rec.Par.Labels.TFEfactor = TFE; end % ZN: for MP2RAGE, the TFE factor shouldn't be changed
...
% %%% Plot traj in number and shots
% permTraj=[3 2 1]; %YB: At this point becomes:readout is in 3rd dimension; Becomes LR-AP-HF (see invert7T.m and main_recon.m)
% kTraj=zeros([length(rec.Assign.z{2}) 2],'single');
% for n=1:2;kTraj(:,n)=rec.Assign.z{permTraj(n)}(:);end
% kTrajSS=reshape(kTraj,[180 77 2]);% YB: SS for snapshot I think
% visTrajectory(kTrajSS,0,[],[]);
...
%     rec.Enc.UnderSampling.Acq.FOVSizeACS = size(rec.ACS,[1 2 3]); % ZN added, to get ACS's orientation changed due to phaseOS or partial Fourier
%     %Assign new values
%     rec.Enc.FOVSize(2:3) = round(rec.Enc.UnderSampling.RTarget .* rec.Enc.AcqSize(2:3));
%     rec.Enc.FOVSizeACS = rec.Enc.UnderSampling.Acq.FOVSizeACS; rec.Enc.FOVSizeACS(2:3) = round(rec.Enc.UnderSampling.Acq.FOVSizeACS(2:3)./(1+PEOverSamp));  % ZN added
...
%             % the following is the original Yannick's code (ZN noted) ->
%             % this doesn't change orientation of ACS accordingly
...
% %             % ZN: change according to Lucilio's code
% %             vr=1:rec.Enc.FOVSize(1);
% %             % high-res
% %             tI=rec.Par.Mine.APhiRec*[vr(1)-1;-(rec.Enc.UnderSampling.Acq.FOVSize(2:3)-rec.Enc.FOVSize(2:3))';1];
% %             rec.Par.Mine.APhiRec(1:3,4)=tI(1:3);
% %             % ACS
% %             tI=rec.Par.Mine.APhiACS*[vr(1)-1;-(rec.Enc.UnderSampling.Acq.FOVSizeACS(2:3)-rec.Enc.FOVSizeACS(2:3))';1];
% %             rec.Par.Mine.APhiACS(1:3,4)=tI(1:3);
%         else%Pad
%             padDim = [0 0 0];
