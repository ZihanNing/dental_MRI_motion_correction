...
function rec = TWIX2recSPACE(TW, suppFOV, isPT, shimDir, removeOversampling, noiseFileName, fileName, resRec, useGPU, removeZeroSamples, targetR, idTE, idINV)
...
isShotBased = strcmp(TW.hdr.Meas.tScanningSequence,'GR\IR') || strcmp(TW.hdr.Meas.tScanningSequence,'SE\IR');% TW.hdr.MeasYaps.sFastImaging.lTurboFactor~=1;
isShotBased = 0
isSPACE = strcmp(TW.hdr.Meas.tScanningSequence,'SE')==1;
...
%FOV array size 
NImageCols = TW.hdr.Config.NImageCols;%
...
NKSpaceCols=TW.image.NCol%SPACE
...
NKSpaceColsGrid = TW.hdr.Meas.iRoFTLenWithout2DInterpolation;
...
%NKSpaceLinsGrid = 2 * ceil(NKSpaceLinsGrid/2)
...
if 0 && any(idDataCorr)
    ...
    corrFac = onesL(dynInd(rec.y,[1],1));
    corrFac = dynInd(corrFac,tt,3, idDataCorrT);
...
% xx = dynInd(rec.y,1,4);
% xx = dynInd(xx, centerIdx(multDimSize(xx,1))-50,1);
% xx = squeeze(xx);
    
% fprintf('SPACE RAW DATA CORRECTION - hard-coded.\n');
% id = [];

% id{1} = 73:77
% id{2} = 95:99
% corrFac = resPop(rec.Par.preProcessing.rawCorrectionScale(:),1,[],4)
% rec.y = dynInd(rec.y,id,2:3,dynInd(rec.y,id,2:3).*corrFac);

% fprintf('SPACE RAW APODISATION - hard-coded.\n');
% facFOV = (43 - centerIdx(size(rec.y,2)))/centerIdx(size(rec.y,2));
% facFOVTh = (60 - centerIdx(size(rec.y,2)))/centerIdx(size(rec.y,2));
% hApod = makePTApod(size(rec.y,2), facFOV,facFOVTh); 
% rec.y = rec.y.*hApod(:).';
...
if isPT
    ...
    rec.PT.yProjRO = multDimMea( abs( dynInd(rec.y,{':',':'},2:3 )),2:4); %k-space centre averaged across coils        
...
rec.Par.Labels.echoSpacing = TW.hdr.MeasYaps.sFastImaging.lEchoTrainDuration;
rec.Par.Labels.RepetitionTime_Long = rec.Par.Labels.RepetitionTime ;%In ms;
rec.Par.Labels.RepetitionTime = rec.Par.Labels.echoSpacing/1000;%In ms;
