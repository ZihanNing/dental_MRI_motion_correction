...
%   * {TARGETR} is the undersampling to change the acquisition to.
...
isMP2RAGE = isShotBased && length(Lin)>prod(TW.image.dataSize(3:4));
isMPRAGE = isShotBased && ~isMP2RAGE;
if isMP2RAGE
...
assymEcho = multDimSum(kSpacePad(:,1))>0;if assymEcho;warning('TWIX2Rec:: Assumetric echo acquired.');end
...
%% NOISE READOUT
typ2Rec={'y'};
if rec.Enc.UnderSampling.Flag && strcmp(rec.Enc.UnderSampling.accelType,'Non-Integrated');typ2Rec{end+1}='ACS';end
...
if isPT
    ...
    yProjRO = fillCent(rec.PT.yProjRO, 1, 0, rec.PT.extraction.enforceUpperFOV);%This assumes the PT is set outside the FOV (and optionally in the upper part of the over-sampled FOV)
    ...
    if rec.PT.extraction.sincInterp.Flag%PT signal is peak of the sinc-interpolated signal
        fprintf('Using sinc interpolation to detect the peak.\n');
        dimImageDomain = 1;
        ...
        if rec.PT.deModulate.Flag && isequal(rec.PT.extraction.sincInterp,rmfield(rec.PT.deModulate,'enforceUpperFOV'))
            fprintf('Using sinc interpolation to de-modulate imaging k-space combined with PT extraction.\n');
            [rec.y, rec.PT.pSliceImage] = demodulatePTLeakage(rec.y, 1, rec.PT.extraction.sincInterp.padFac,  rec.PT.extraction.sincInterp.windowCent, rec.PT.extraction.sincInterp.windowWidth, [], rec.PT.extraction.enforceUpperFOV,dimImageDomain,LinTemp,ParTemp);
        elseif rec.PT.deModulate.Flag
            fprintf('Using sinc interpolation to de-modulate imaging k-space.\n');
            [~, rec.PT.pSliceImage] = demodulatePTLeakage(rec.y, 1, rec.PT.extraction.sincInterp.padFac,  rec.PT.extraction.sincInterp.windowCent, rec.PT.extraction.sincInterp.windowWidth, [], rec.PT.extraction.enforceUpperFOV,dimImageDomain,LinTemp,ParTemp);
            rec.y = demodulatePTLeakage(rec.y, 1, rec.PT.deModulate.padFac,  rec.PT.deModulate.windowCent, rec.PT.deModulate.windowWidth, [], rec.PT.deModulate.enforceUpperFOV,dimImageDomain,LinTemp,ParTemp);
        else
            [~, rec.PT.pSliceImage] = demodulatePTLeakage(rec.y, 1, rec.PT.extraction.sincInterp.padFac,  rec.PT.extraction.sincInterp.windowCent, rec.PT.extraction.sincInterp.windowWidth, [], rec.PT.extraction.enforceUpperFOV,dimImageDomain,LinTemp,ParTemp);
        end
        rec.PT.offsetMBHz=0;
        ...
        fprintf('Using biggest element in hybrid domain.\n');
        ...
        if rec.PT.deModulate.Flag
            fprintf('Using sinc interpolation to de-modulate imaging k-space.\n');
            rec.y = demodulatePTLeakage(rec.y, 1, rec.PT.deModulate.padFac,  rec.PT.deModulate.windowCent, rec.PT.deModulate.windowWidth, [], rec.PT.deModulate.enforceUpperFOV,dimImageDomain,Lin,Par);
        end
    end
...
if removeZeroSamples && (rec.Enc.UnderSampling.Flag || ~isempty(targetR))  && ~isequal(rec.Enc.UnderSampling.RTarget,rec.Enc.UnderSampling.R) 
    ...
    for l=2:3
        ...
        if rec.Enc.UnderSampling.Acq.FOVSize(l)>rec.Enc.FOVSize(l)%Extract
            ...
            padDim = [0 0 0];
            padDim(l) = (centerIdx(rec.Enc.UnderSampling.Acq.FOVSize(l))  - centerIdx(rec.Enc.FOVSize(l))) ;%Only need padding before
...
%     padLin = (centerIdx(rec.Enc.UnderSampling.Acq.FOVSize(2))  - centerIdx(rec.Enc.FOVSize(2))) ;
%     padLin = cat(2, padLin, rec.Enc.UnderSampling.Acq.FOVSize(2)-rec.Enc.FOVSize(2)-padLin );
%     padPar = (centerIdx(rec.Enc.UnderSampling.Acq.FOVSize(3))  - centerIdx(rec.Enc.FOVSize(3))) ;
%     padPar = cat(2, padPar, rec.Enc.UnderSampling.Acq.FOVSize(3)-rec.Enc.FOVSize(3)-padPar );
%     [rec.Enc.AcqVoxelSize , rec.Par.Mine.APhiRec] = mapNIIGeom(rec.Enc.AcqVoxelSize , rec.Par.Mine.APhiRec,'padArrayND',[0 padLin(1) padPar(1)],rec.Enc.UnderSampling.Acq.FOVSize,rec.Enc.FOVSize);
%     [~,rec.Par.Mine.APhiRecOrig]=mapNIIGeom([],rec.Par.Mine.APhiRecOrig,'ipermute',rec.Par.Mine.permuteHist{1});
%     [~,rec.Par.Mine.APhiRecOrig]=mapNIIGeom([],rec.Par.Mine.APhiRecOrig,'padArrayND',[0 padLin(1) padPar(1)],rec.Enc.UnderSampling.Acq.FOVSize,rec.Enc.FOVSize);
%     [~,rec.Par.Mine.APhiRecOrig]=mapNIIGeom([],rec.Par.Mine.APhiRecOrig,'permute',rec.Par.Mine.permuteHist{1});
%     fprintf('Changing undersampling from R=%.1fx%.1f to R=%.2f-%.2f by changing the FOV size.\n', rec.Enc.UnderSampling.Acq.R(1),rec.Enc.UnderSampling.Acq.R(2),rec.Enc.UnderSampling.R(1),rec.Enc.UnderSampling.R(2));
 end
...
if ~isempty(resRec)
    ...
    rec.Par.preProcessing.resRec.resNew = resRec;
end
