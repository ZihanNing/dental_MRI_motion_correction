...
function rec = TWIX2recTest(TW, suppFOV, isPT, shimDir, removeOversampling, noiseFileName)

%TWIX2REC   Inverts SIEMENS TWIX data to a reconstruction structure.
%   * TW is the TWIX object coming from the mapVBVD.m function
%   * {SUPPFOV} is the support in the readout direction.
%   * {ISPT} a flag to show the data contains PT signal which needs to be extracted (defaults to 0).
%   * {SHIMDIR} is directory with additional shim calibration information of the scanner.
%   * {REMOVEOVERSAMPLING} is a flag whether to remove the Siemens default oversampling.
...
elseif isPT
    if exist(strcat( noiseFileName, '.mat'),'file')==2
        ss = load(noiseFileName);fprintf('Using noise samples from separate acquisition: %s\n',noiseFileName)
        if isfield(ss.rec,'N');rec.N = ss.rec.N;
        else; warning('TWIX2rec:: No noise samples found in noiseName provided. Noise de-correlation disabled!');
        end
        rec.PT.noiseFileName = noiseFileName;
        clear ss;
    else
        warning('TWIX2rec:: No file noiseName found. Noise de-correlation disabled!');
    end    
end
...
if isPT
    ...
    rec.PT.yProjRO = multDimSum(abs(rec.y),[2:4]); %Not in the readout
        zeroF=ceil((NY+1)/2);
    ...
    yProjRO = dynInd(rec.PT.yProjRO, idxTemp, 1, 0);%Set FOV to zero. This assumes the PT is set outside the FOV
    ...
    rec.PT.FWHMHz = getFWHM( BWVox * (1:length(yProjRO)) , yProjRO');
...
if ~isempty(suppFOV)
    NY = size(rec.y);
    vr=round(NY(1)*suppFOV(1)):round(NY(1)*suppFOV(2)); %NY(1) is the readout
...
for n=2:3
    ...
    rec.y=ifftGPU(rec.y,n);%*NY(l);
    rec.y=fftshiftGPU(rec.y,n);
    
    if isPT
        rec.PT.pSliceImage=ifftshiftGPU(rec.PT.pSliceImage,n);
        rec.PT.pSliceImage=ifftGPU(rec.PT.pSliceImage,n);%*NY(l);
...
if isPT
    ...
    subplot 121; plot(1:size(yProjRO,1), multDimSum(abs(rec.PT.yProjRO),[2:4])');hold on; scatter(rec.PT.idxRO,rec.PT.yProjRO(rec.PT.idxRO));axis tight
    title('Untouched hybrid k-space')

    subplot 122; plot(1:size(yProjRO,1), yProjRO');hold on; scatter(rec.PT.idxRO,yProjRO(rec.PT.idxRO));axis tight
    title('FOV set to 0')
...
%% DE-CORRELATE NOISE
if isfield(rec,'N')
    [rec.y, covMatrix] = standardizeCoils(rec.y,rec.N);
...
if exist('covMatrix','var'); figure('color','w'); imshow(abs(covMatrix),[]);title('Noise covariance matrix.');end
...
N = size(rec.y); N=N(1:3);%N(2) will depend if you removed over-sampling
...
[ rec.Enc.AcqVoxelSize  ,  rec.Par.Mine.APhiRec] = mapNIIGeom( rec.Enc.AcqVoxelSize , rec.Par.Mine.APhiRec,'flip',1:3,NY,NY );
[ rec.Enc.AcqVoxelSize  ,  rec.Par.Mine.APhiRec] = mapNIIGeom( rec.Enc.AcqVoxelSize , rec.Par.Mine.APhiRec,'translate',[-1 -1 -1],NY,NY);
...
%ATTENTION: this mod() operator gives big spikes in the sampling. Questions
%whether this a real thing or just processing....

rec.Assign.z{2}= (0*NY(2) + (Lines-0)) - kShift(2); % 2nd PE 
rec.Assign.z{3}= (0*NY(3) + (Partitions-0)) - kShift(3); % 3rd PE = slices
...
if isPT
    ...
    figure('color','w'); coilPlot=25;
    subplot 211 ; plot(1:size(rec.PT.pTimeTest,2),abs(dynInd(rec.PT.pTimeTest,coilPlot,1).' ));
    xlabel('Time (PE readouts) [TR]'); ylabel('Pilot Tone signal magnitude [a.u]'); axis tight
    subplot 212 ; plot(1:size(rec.PT.pTimeTest,2),angle(dynInd(rec.PT.pTimeTest,coilPlot,1).' ));
    xlabel('Time (PE readouts) [TR]'); ylabel('Pilot Tone signal phase [a.u]'); axis tight    
end
...
if isPT
    centreFOV = rec.Par.Mine.APhiRec * [ceil( (NY(1:3)-1 -.5 +1)/2) 1]';%-1 since NIFTI convention and not MATLAB
    centreFOV = rec.Par.Mine.APhiRec * [(NY(1:3)/2 -[ 0 0 .5]) 1]';%-1 since NIFTI convention and not MATLAB
    ...
    rec.PT.transmitFreqHz = setPTFreq(...
       rec.Par.Labels.Shim.shimCurrents_au(1)/1e6 ,...
       NImageCols,...
