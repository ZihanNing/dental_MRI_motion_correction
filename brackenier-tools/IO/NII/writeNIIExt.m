...
function writeNIIExt(dat,suffix,x,MS,MT,underScore)

%WRITENIIEXT   Writes a set of nii files with the option to leave out the underscore before the suffixes. Adapted from writeNII.m.
%   WRITENIIEXT(DAT,SUFF,X,{MS},{MT},{UNDERSCORE})
...
if nargin < 6 || isempty(underScore); underScore=0;end
...
for n=1:N    
    ...
    if any(imag(x{n}(:)))
        ...
        if underScore; save_nii(niftiIm,sprintf('%s_%sPh.nii',dat,suffix{n}));else; save_nii(niftiIm,sprintf('%s%sPh.nii',dat,suffix{n}));end
        x{n}=abs(x{n});
    end
    x{n}=gather(real(x{n}));    
    ...
    niftiIm.hdr.hist.srow_x=MT{n}(1,:);niftiIm.hdr.hist.srow_y=MT{n}(2,:);niftiIm.hdr.hist.srow_z=MT{n}(3,:);niftiIm.hdr.hist.sform_code=1;            
    if length(MS{n})>=4;niftiIm.hdr.dime.pixdim(5)=MS{n}(4);end
    if underScore; save_nii(niftiIm,sprintf('%s_%s.nii',dat,suffix{n}));else; save_nii(niftiIm,sprintf('%s%s.nii',dat,suffix{n}));end
end
