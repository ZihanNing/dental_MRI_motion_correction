...
function visMotionExt(rec,voxSiz,t,pau,folderName,fileName,yl,xl,op,numFig,titleName,MT, fontScaling)

%VISMOTIONEXT extendss the visMotion.m function to visualizes motion estimates.
%   VISMOTIONEXT(REC,{VOXSIZ},{PAU},{FOLDERNAME},{FILENAME},{FILENAME},{YL},{XL},{OP},{NUMFIG},{TITLENAME},{MT})
%   * REC is a reconstruction structure that contains motion states in rec.T.
%   * {VOXSIZ} is the spacing of the image space.
%   * {T} is the time of the motion states.
%   * {PAU} serves to pause the execution (defaults to 0).
%   * {FOLDERNAME} gives a folder where to write the results.
%   * {FILENAME} gives a file where to write the results.
%   * {YL} are the limits for translations (first row) and rotations (second row).
%   * {XL} are the temporal limits.
%   * {OP} serves to set a different opacity for different segments in the xaxis.
%   * {NUMFIG} the figure number. If negative, it will not create a new figure.
%   * {TITLENAME} a string that contains the title.
%   * {MT} the s-form orientation in which the motion parameters are defined.
%
%   Yannick Brackenier 2023-09-08 - modified code from Lucilio Cordero-Grande
%

if nargin<2 || isempty(voxSiz);voxSiz=[];end
if nargin<3 || isempty(t);t=[];end
if nargin<4 || isempty(pau);pau=0;end
if nargin<5 || isempty(folderName);folderName=[];end
if nargin<6 || isempty(fileName);fileName=[];end
if nargin<7 || isempty(yl);yl=[];end
if nargin<8 || isempty(xl);xl=[];end
if nargin<9 || isempty(op);op=[];end
if nargin<10 || isempty(numFig);numFig=[];end
if nargin<11 || isempty(titleName);titleName='';end
if nargin<12 || isempty(MT);MT=[];end
if nargin<13 || isempty(fontScaling);fontScaling=[1 1 1 1];end
...
FontSizeA=30*fontScaling(1);
ModLab=15*fontScaling(2);
ModTit=15*fontScaling(3);
LineWidth=6*fontScaling(4);
...
if isfield(rec,'T')
    ...
    if ~isempty(op) && size(t,3)==2
        for l=1:2*size(t,1)
            ...
            if l<=size(t,1)+1;LineW=LineWidth;els;e LineW=2*LineWidth/3;end
    ...
    %%% ORIENTATION
    if ~isempty(MT)
        temp = acquisitionOrder(MT);
        dire{1} = temp(1:2);
        dire{2} = temp(4:5);
        dire{3} = temp(7:8);
        for m=4:6;dire{m}= dire{m-3};end
        if isfield(rec,'Par');fprintf('visMotionExt:: Using MT instead of rec.Par to infer geometry.\n');end
        
    elseif isfield(rec,'Par')
    ...
    AX = legend(dire);
    LEG = findobj(AX);
    set(LEG,'Color','none','TextColor',[1 1 1]*(1-invCol),'Location','NorthWest','FontSize',FontSizeA,'Interpreter','latex')         
    ...
    %%% TITLE
    if ~strcmp(titleName,''); title(titleName,'Interpreter','latex','Color',[1 1 1]*(1-invCol),'FontSize',FontSizeA+ModTit);end
end

if pau==1;pause;end
if isempty(folderName) && ~isempty(fileName)%WE SIMPLY WRITE TO FILE
