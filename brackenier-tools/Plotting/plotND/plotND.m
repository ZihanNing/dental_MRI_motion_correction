...
function [h, hColorbar, X, B] = plotND(xRef, x, limPlot, idPlot, cmapInfo, sliceInfo, ...
                                       MT, labelText, M, usageM, numFig, ...
                                       titleText, colorbarText, FontSizeScaling,...
                                       folderName, fileName)
...
%   * {M} the mask to use for plotting. If plotting contours with M (USAGEM{1}==2), multiple contours are stored by makin M a bitmask.
...
%             USAGEM{2} is a cell array itself with the color for each contour.
%             USAGEM{3} the contour linewidth.
%             USAGEM{4} a smoothing factor (from 0 to 1 with 1 almost no effect and 0 fully smoothed). No smoothing is USAGEM{4}==inf.
...
%% Make sure x does not exceed 5 dimensions
N = size(x);
if length(N)>5; x = squeeze(x);warning('plotND:: More than 5 dimensions detected so x has been squeezed.'); end
...
%%% limPlot
if isempty(limPlot);limPlot=getRange(x);end
...
if isscalar(cmapInfo)
    ...
    else; error('plotND:: cmapInfo scalar not implemented.'); 
    end
elseif ismatrix(cmapInfo) && size(cmapInfo,1)>1
    cMap = cmapInfo; assert(size(cMap,2)==3,'plotND:: cmapInfo matrix containing colormap not valid.');
elseif ischar(cmapInfo)
    cMap = colormap(cmapInfo);
elseif iscell(cmapInfo)
    cMap = createColormap(cmapInfo);
end
...
%% ADD REFERENCE IMAGE (xRef) - and stack it in the right dimension
colorRef = 1;
if ~isempty(xRef)
    if iscell(xRef)
...
%% PERMUTE TO RAS (Right-Anterior-Superior) to have consistent imaging planes
if ~isempty(MT)
...
%% STACK IMAGE PLANES
tt=[];for ii=sliceInfo{1};tt= [tt setdiff(1:3, ii) ] ;end
tt=max(N(tt));
Ni = tt * ones(1,2);%Don't resample but use margins (avoids Gibsringing artefacts)
...
for vert = 1:N(4)
    ...
    for hor=1:N(5)
        ...
        for d=sliceInfo{1} %which slices to plot
            perm = 1:3;
            perm = circshift(perm,-d);
            z = permute(y,perm);
            if usageM{1}>=2; m=permute(M,perm);end%For contour plotting or color masking
            ...
            z = resampling(z,Ni,2);    
            if usageM{1}>=2; m = resampling(m,Ni,2);end%single(resampling(m,Ni,2) > 0.5)
...
if ~(nargout > 2) %If stacked array with images is requested, don't plot
    ...
    %%% FIGURE
    h=createFig(numFig);
    if numFig>0;set(axes,'Position',[0.005,0.005,0.95,.95]);end%subtightplot(1,1,1,0,.03,0.03);%To ensure there is a margin to show colorbar/colorbar text
    ...
    elseif usageM{1}==4 %Transparant mask
        if length(usageM)<2 || isempty(usageM{2}); usageM{2} = .5;end%Opacity
        rgbMask = repmat(onesL(X),[1 1 3]);
        rgbMask = resSub(rgbMask,1:2);
        rgbMask(Xm==1,:) = cat(2,onesL(X(Xm==1)),(1-usageM{2}.*Xm(Xm==1)),(1-usageM{2}.*Xm(Xm==1)));
        rgbMask(Xm==2,:) = cat(2,(1-usageM{2}.*Xm(Xm==2)),onesL(X(Xm==2)),(1-usageM{2}.*Xm(Xm==2)));
        rgbMask = resSub(rgbMask,1,size(Xm));
    ...
    elseif usageM{1}==5 %Transparant mask
        plotMasked(X, limPlot, cMap, Xm, XmGrey,[], 1);
    else
        imshow(X,limPlot);
    end
    ...
    if ~useColor || usageM{1}==3; color='w'; else; color = [0 0 0]; end %black or white / orange([0.8500 0.3250 0.0980])
    for i=1:N(4)
        for j = 1:N(5)
    ...
    if usageM{1}==2
        ...
        numMasks = floor( log(multDimMax(Xm))/log(2) ) + 1;
        for contourId = 1:numMasks
            XmTemp = single(bitget(uint8(Xm),contourId));
            if ~isinf(sp)
                HY = buildFilter(2*size(Xm),'tukeyIso',sp,0,0.3,1);%In DCT domain
                XmTemp = single( filtering(single(XmTemp), HY, 1) > 0.5);%Make contour smoother
            end
            B = bwboundaries(gather(XmTemp==1));

            figure(h); hold on
            for k = 1 : length(B)
                 thisBoundary = B{k};  % Get k'th boundary
                 x = thisBoundary(:, 2); y = thisBoundary(:, 1);
                 plot(x, y, 'color',c{contourId}, 'LineWidth', Linewidth);
            end
        end
        hold off;
    else
