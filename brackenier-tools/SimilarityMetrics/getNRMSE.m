...
function NRMSE = getNRMSE(x, xGT, ROI, useMagn)

%getNRMSE computes the Normalised Root Mean Squared Error (MSE) between an image and the ground truth (GT).
%   [MSE] = GETNRMSE(X,XGT,{ROI}) 
...
%   ** NRMSE the calculated NRMSE.
%
%   Yannick Brackenier 2023-11-07

if nargin < 2 || isempty(xGT); error('getNRMSE:: Ground Truth array expected.'); end
if nargin < 3 ; ROI = []; end
