...
elseif  strcmp(type,'R2T')
    ...
    flagValid = abs( det(R)-1 ) <= 1e-5;
    if ~flagValid
        R
        det(R)-1
    end
    assert(flagValid,'convertT :: Not a valid transformation matrix.');
    ...
    if useSpinCalc    
        ...
        EA = pi/180*SpinCalc(sprintf('DCMtoEA%s', eulerOrder),inv(R(1:3,1:3)),tol,~suppressWarning); %Transpose because of convention SpinCalc.m
        EA = mod(EA+pi, 2*pi)-pi;
    else
        EA = zeros([1 3],'like',real(Tin));
        EA(3)=atan2(-R(1,2), R(1,1));%z-component
        EA(2)=asin(R(1,3));%y-component
        EA(1)=atan2(-R(2,3), R(3,3));%x-component
    end
